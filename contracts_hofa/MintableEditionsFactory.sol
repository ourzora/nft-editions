...
contract MintableEditionsFactory {
  ...
  //  Counters.Counter private atContract;

    /// Address for implementation of MintableEditions to clone
    address public implementation;

    /// Initializes factory with address of implementation logic
    /// @param _implementation MintableEditions logic implementation contract to clone
    ...
    /*    uint256 newId = atContract.current();
        ...
        MintableEditions(newContract).initialize(
            msg.sender,
            _name,
            _symbol,
            _description,
    ...
    /// @return MintableEditions Edition NFT contract
    function getEditionAtId(uint256 editionId)
        external
        view
        returns (MintableEditions)
    {
        return
            MintableEditions(
